...
# ─────────────────────────────────────────────────────────────────────────────
#  ODCZYT CZUJNIKA – BMP280 (Temperatura i Ciśnienie)
# ─────────────────────────────────────────────────────────────────────────────
def read_sensor_data() -> tuple[float | None, float | None]:
    """Odczytuje temperaturę (z korektą) oraz ciśnienie z BMP280.
    Po MAX_SENSOR_ERRORS kolejnych błędach próbuje reinicjalizować czujnik.
    Zwraca (temperatura, ciśnienie) lub (None, None) przy błędzie."""
    global sensor, sensor_error_cnt
    if sensor is None:
        return None, None
    try:
        temp = round(sensor.temperature + TEMP_CORRECTION, 2)
...
def _append_row(path: str, row: list, write_header: bool) -> None:
    ...
    with open(path, mode="a", newline="", encoding="utf-8") as f:
        ...
        try:
            writer = csv.writer(f)
            if write_header:
                writer.writerow(["timestamp", "temp_bmp_c", "pressure_hpa"])
            writer.writerow(row)
            f.flush()
            os.fsync(f.fileno())
        finally:
...
def write_to_csv() -> bool:
    ...
    bmp_temp, pressure = read_sensor_data()

    if bmp_temp is None:
        logger.warning(f"{timestamp} – Brak odczytu BMP280, pomijam zapis do CSV.")
        return False

    row = [timestamp, bmp_temp, pressure if pressure is not None else ""]
    ok  = True
    ...
    if ok:
        logger.info(
            ...
            f" | BMP={bmp_temp}°C | Ciśnienie={pressure} hPa"
        )
    return ok
...
def push_with_timeout(origin) -> None:
    ...
    Sprawdza wynik PushInfo – rzuca wyjątek gdy push został odrzucony."""
    result_box = [None]
    error_box  = [None]
    t = threading.Thread(
    ...
    # Sprawdź flagi PushInfo – gitpython nie rzuca wyjątku przy rejected!
    push_infos = result_box[0]
    if push_infos:
        # Flagi błędów z gitpython (bitowe OR)
        ERROR_FLAGS = (
        ...
        for info in push_infos:
            if info.flags & ERROR_FLAGS:
                raise git.exc.GitCommandError(
                    "git push",
                    128,
                    stderr=getattr(info, 'summary', str(info))
                )
...
def commit_and_push() -> None:
    ...
    try:
        ...
        if "non-fast-forward" in msg or "rejected" in msg:
            ...
            try:
                repo = git.Repo(BASE_DIR)
                # Jawne --rebase żeby git nie pytał o strategię merge
                repo.git.execute(["git", "pull", "--rebase", "--quiet"])
...
def main() -> None:
    ...
    # Ustaw licznik tuż przed progiem commita – pierwszy pomiar wywoła commit od razu
    measurement_cnt = COMMIT_EVERY_N - 1
    logger.info("Pierwszy commit nastąpi po pierwszym pomiarze.")
